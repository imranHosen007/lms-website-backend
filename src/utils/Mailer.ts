...
interface ActivateEmailOpetions {
  name: string;
  email: string;
  subject: string;
...
export const ActivateSendMail = async (
  options: ActivateEmailOpetions
): Promise<void> => {
...
// ------Order-ConfirmationMail----

interface ConfirmationEmailOpetions {
  id: any;
  name: string;
  subject: string;
  price: number;
...
export const OrderConfirmationMail = async (
  options: ConfirmationEmailOpetions
): Promise<void> => {
  const transport: Transporter = nodemailer.createTransport({
    host: process.env.SMTP_HOST,
    port: parseInt(process.env.SMTP_PORT || "5000"),
